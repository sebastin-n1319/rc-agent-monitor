...
function deskFetch(pathAndQuery, token, { includeOrgId = true } = {}) {
  const headers = { Authorization: `Zoho-oauthtoken ${token}` };
  if (includeOrgId) headers.orgId = ZOHO_DESK_ORG_ID;
  return fetch(`${ZOHO_API_BASE}${pathAndQuery}`, { headers });
}
...
async function deskGet(pathAndQuery) {
  ...
  const res = await deskFetch(pathAndQuery, token);
  if (res.status === 429) {
  ...
  if (res.status === 403) {
    ...
    // The primary Desk token was granted only ticket-level scopes, not the
    // full Desk scope set -- the suite token (built for CRM+Desk+Analytics+
    // SalesIQ together) has everything, so retry through it once before
    // giving up. Fixes this at the source for every deskGet() caller
    // (ticket search, metrics, history, agent/department lookups) instead
    // of needing a bespoke fallback wired into each one.
    if (text.includes('SCOPE_MISMATCH') && analyticsService.isConfigured()) {
      console.warn(`⚠️ Primary Desk token missing scope for ${pathAndQuery.split('?')[0]} -- retrying via suite token`);
      try {
        const suiteToken = await analyticsService.getSuiteAccessToken();
        // Verified live: ZOHO_DESK_ORG_ID (the primary token's org) makes
        // the suite token's requests silently return zero results instead
        // of erroring -- omit orgId on this retry, same as
        // analyticsService.fetchDeskDepartments() already does successfully.
        const res2 = await deskFetch(pathAndQuery, suiteToken, { includeOrgId: false });
        if (!res2.ok) {
          const text2 = await res2.text().catch(() => '');
          throw new Error(`Zoho Desk API ${res2.status}: ${text2.slice(0, 300)}`);
        }
        return res2.json();
      } catch (e2) {
        throw new Error(`Zoho Desk API 403: ${text.slice(0, 300)} (suite token retry also failed: ${e2.message})`);
      }
    }
    throw new Error(`Zoho Desk API 403: ${text.slice(0, 300)}`);
  }
  if (!res.ok) {
    const text = await res.text().catch(() => '');
 ...
 *
 *  Session 21 rebuild: the search window is now an explicit
 *  [fromTimeMs, toTimeMs) bound instead of a fixed rolling lookback --
 *  the old fixed-window-plus-from=0-every-tick design is what plateaued
 *  the sync at whatever fit in one window, re-fetched identically every
 *  tick, regardless of what date range the UI asked for (see
 *  server.js#runDeskLifecycleSync for the resumable two-phase caller
 *  that shrinks/advances this window tick over tick). `lookbackDays` is
 *  kept only as a fallback default for a caller that doesn't pass
 *  explicit bounds. */
async function fetchTicketsPage({ departmentId, from = 0, limit = 50, lookbackDays = 100, fromTimeMs = null, toTimeMs = null } = {}) {
  const range = (fromTimeMs != null && toTimeMs != null)
    ? `${new Date(fromTimeMs).toISOString()},${new Date(toTimeMs).toISOString()}`
    : isoRange(lookbackDays);
  const query = qs({
    departmentId,
    from,
    limit,
    sortBy: '-modifiedTime',
    modifiedTimeRange: range,
  });
  const data = await deskGet(`/tickets/search${query}`);
  // Verified live against the real Zoho response: /tickets/search returns
  // { data: [...tickets], count: N } -- ONE level of `.data`, not two. The
  // previous `data.data.data` always evaluated to undefined (arrays have
  // no `.data` property), so this silently returned [] on every call even
  // once the auth issue above was fixed -- that's why the sync kept
  // reporting 0 tickets synced with no error.
  return data.data || [];
}
...
// Session 21 fix: unlike /tickets/search (which wraps results in
// { data: [...] }), /tickets/{id}/metrics returns the metrics object
// directly at the top level -- no `.data` wrapper. `data.data` was
// therefore always undefined, so this silently returned null on every
// single call, which is why desk_ticket_agents (and reassign/reopen/
// resolution-hours from metrics) stayed permanently empty despite
// tickets syncing fine -- live-confirmed against the real API via the
// admin debug-metrics/debug-customer-feedback?path= routes.
