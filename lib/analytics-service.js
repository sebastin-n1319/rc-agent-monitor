...
// Session 40: "Tickets (Zoho Desk)" base table in the same workspace as the
// survey table above -- confirmed via the workspace's view listing (id
// 1439480000001653277, viewType "Table"). Holds Zoho's own authoritative
// "Resolution Time in Business Hours" (milliseconds) and "Is First Call
// Resolution" columns per ticket -- see fetchTicketFcrRows() below for why
// this app needs them instead of computing its own.
const ZOHO_ANALYTICS_TICKETS_VIEW_ID = process.env.ZOHO_ANALYTICS_TICKETS_VIEW_ID || '1439480000001653277';
...
/** Session 40: per-ticket FCR ground truth, sourced from Zoho Analytics'
 *  own "Tickets (Zoho Desk)" table instead of reconstructed from raw
 *  created/closed timestamps (see lib/desk-lifecycle.js's FCR query for
 *  why -- Zoho's internal SLA/business-hours engine pauses the resolution
 *  clock during statuses this app has no visibility into, confirmed by
 *  cross-checking against Sebastin's own methodology export; the public
 *  Desk REST API's /tickets/{id}/metrics does not expose either field,
 *  confirmed against Zoho's own OpenAPI spec).
 *
 *  Pulls BOTH:
 *   - "Resolution Time in Business Hours" (raw milliseconds in Analytics,
 *     converted to hours here) -- kept for reporting/SLA uses beyond FCR.
 *   - "Is First Call Resolution" -- Zoho's own per-ticket FCR flag. This is
 *     NOT always equal to (resolution_business_hours <= 24 AND
 *     reopen_count == 0): spot-checked examples found tickets where Zoho's
 *     flag disagrees with that rule even given the correct hours number
 *     (e.g. a sub-1-hour, never-reopened ticket marked FCR=No; a ticket
 *     reopened once still marked FCR=Yes, most likely because Zoho
 *     evaluates FCR at the ticket's original close, before a later
 *     reopen, while this app's reopen_count reflects current state). Per
 *     Sebastin's decision, this flag -- not the reconstructed rule -- is
 *     what desk-lifecycle.js's FCR% now counts against.
 *
 *  `fromChicagoStr`/`toChicagoStr` are naive "YYYY-MM-DD HH:MM:SS" wall-
 *  clock strings in the table's own display timezone (America/Chicago,
 *  same convention as "Survey Time" -- see file header and
 *  utcIsoToChicagoWallTime()), scoping the pull to a "Ticket Closed Time"
 *  window so a single call never has to page the full multi-hundred-
 *  thousand-row table (this API has no offset/limit pagination -- see
 *  server.js's runFcrAnalyticsSync() for how the caller walks this in
 *  bounded windows). Both bounds are required. */
async function fetchTicketFcrRows(fromChicagoStr, toChicagoStr) {
  const token = await getAccessToken();
  const config = {
    responseFormat: 'json',
    selectedColumns: ['ID', 'Resolution Time in Business Hours', 'Is First Call Resolution', 'Ticket Closed Time'],
    criteria: `"Ticket Closed Time" >= '${fromChicagoStr}' AND "Ticket Closed Time" < '${toChicagoStr}'`,
  };
  const url = `${ZOHO_ANALYTICS_BASE}/workspaces/${ZOHO_ANALYTICS_WORKSPACE_ID}/views/${ZOHO_ANALYTICS_TICKETS_VIEW_ID}/data?CONFIG=${encodeURIComponent(JSON.stringify(config))}`;
  const res = await fetch(url, {
    headers: {
      'Authorization': `Zoho-oauthtoken ${token}`,
      'ZANALYTICS-ORGID': ZOHO_ANALYTICS_ORG_ID,
  ...
  const data = await res.json();
  if (!res.ok || data.status === 'failure') {
    throw new Error(`Zoho Analytics tickets export failed: ${(data.data && data.data.errorMessage) || data.summary || res.status}`);
  }
  const rows = data.data || [];
  return rows.map(r => {
    const ms = Number(r['Resolution Time in Business Hours']);
    return {
      ticket_id: r['ID'] != null ? String(r['ID']) : null,
      resolution_business_hours: Number.isFinite(ms) ? ms / 3600000 : null,
      is_fcr: String(r['Is First Call Resolution'] || '').trim().toLowerCase() === 'yes' ? 1 : 0,
      closed_time_utc: centralWallTimeToUtcIso(r['Ticket Closed Time']),
    };
  });
}
