...
// Session 23: default backfill floor raised from 90 days to 730 (~2
// years), matching desk-lifecycle's DESK_BACKFILL_FLOOR_DAYS -- the sync
// itself was already healthy (this two-phase pattern is what desk's own
// sync was modeled on), it was just intentionally shallow. Existing
// deployments need their backfill_complete/backfilled_to_ms sync-state
// reset once (see /api/admin/chat-lifecycle/reset-backfill below) so
// this resumes walking back past the old 90-day stopping point instead
// of staying flagged complete forever.
async function runChatConversationSync({ maxPagesPerPhase = 15, pageSize = 100, initialLookbackDays = 730 } = {}) {
  if (!salesiq.isConfigured()) return { skipped: true };
  const now = Date.now();
  ...
  //
  // Bug fix: conversations come back NEWEST-first (see paginateAndUpsert's
  // comment above), so a tick that can't finish the whole [syncedThrough,
  // now) window always processes the NEWEST slice first and leaves the
  // OLDEST slice -- the part closest to syncedThrough -- unprocessed.
  // Advancing synced_through_ms to anything inside that window (this used
  // to jump to the newest timestamp seen, `maxSeen`) skips straight past
  // that unprocessed older slice forever: synced_through_ms only moves
  // forward, so it's never back in-window on a later tick, and whichever
  // agent handled those chats is silently undercounted permanently. Simply
  // advancing to the OLDEST timestamp seen (`minSeen`) instead doesn't fix
  // it either -- the next tick's window still starts at that same minSeen
  // and (with no new arrivals) reprocesses the identical newest slice,
  // making no progress on the older unprocessed tail.
  //
  // The actual fix: freeze a shrinking ceiling for this catch-up, exactly
  // mirroring how the backfill phase below already handles the same
  // "more data than one tick's budget" problem (see its own comment). While
  // a burst is being worked through, `topup_ceiling_ms` pins the upper
  // bound and each incomplete tick shrinks it down to the oldest item that
  // tick actually saw -- syncedThrough itself never moves until the whole
  // frozen window is fully drained, so nothing between them can ever be
  // skipped. Once a tick finishes the (possibly-shrunk) window, the real
  // watermark advances to that window's ceiling and the freeze clears, so
  // top-up goes back to chasing live "now" next time.
  ...
  const storedTopupCeiling = await getSyncState('topup_ceiling_ms');
  const topupToBound = storedTopupCeiling ? Number(storedTopupCeiling) : now;
  const topUp = await paginateAndUpsert({
    fromTimeMs: syncedThrough, toTimeMs: topupToBound,
    maxPages: maxPagesPerPhase, pageSize, trackMin: true,
  });
  if (topUp.caughtUp) {
    await setSyncState('synced_through_ms', String(topupToBound));
    if (storedTopupCeiling) await setSyncState('topup_ceiling_ms', null);
  } else if (topUp.minSeen != null) {
    await setSyncState('topup_ceiling_ms', String(topUp.minSeen));
  }
