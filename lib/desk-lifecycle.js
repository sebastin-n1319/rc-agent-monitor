...
async function agentTicketsForMetric({ metric, from, to, email, agentName, rosterNames, q, limit = 500 }) {
  ...
  if (ownerFamily.has(metric)) {
    ...
    for (const row of rows) {
      ...
      if (next && next.name.trim().toLowerCase() !== entries[lastIdx].name.trim().toLowerCase()) {
        if (!rosterNameSet.has(next.name.trim().toLowerCase())) transferred = true;
  ...
  if (metric === 'closed' || metric === 'fcr') {
    ...
    return rows.map((row) => ({
      ...row,
      fcr_achieved: row.resolution_business_hours != null && row.resolution_business_hours <= 24 && (row.reopen_count || 0) === 0,
    }));
  }
...
module.exports = {
  ...
  agentSummary, agentTicketList, agentTicketsForMetric, syncStatus,
};
