 ...
 */
async function agentSummary({ from, to, emails, q, agentNames }) {
  ...
  if (nameToEmail.size) {
    ...
    for (const row of ownerRows) {
      let entries = parseOwnerChangeLog(row.log);
      ...
      for (const [email, idxs] of hitIdxByEmail) {
        ...
        if (entries.length === 1) agent.solely_handled++;
        if (entries.length > 1) agent.reassigned++;
...
async function agentTicketList({ email, from, to, limit = 200, q, agentName }) {
  ...
  for (const row of rows) {
    let entries = parseOwnerChangeLog(row.owner_change_log);
    ...
    out.push({
      ...
      // Approximates the ticket's total handoff count from the log itself
      // (entries.length - 1 owner-change events) now that desk_ticket_agents
      // is no longer the source -- not agent-specific, matches the old
      // field's ticket-level meaning.
