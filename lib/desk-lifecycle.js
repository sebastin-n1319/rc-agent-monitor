  ...
  if (nameToEmail.size) {
    ...
    for (const row of ownerRows) {
      ...
      for (const [email, idxs] of hitIdxByEmail) {
        ...
        // agent of other team" (confirmed with Sebastin: a straight
        // T1-to-T1 handoff counts as the receiver's "reassigned" only,
        // not this agent's "transferred"). Checked off their LAST
        // occurrence, so only the hop that actually leaves this agent's
        // hands counts.
