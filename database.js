...
async function getAgentSummary(date){
  ...
  for(const agent of agents){
    ...
    // Inbound calls
    const inCalls=await get(`SELECT COUNT(*) as total, AVG(duration) as avgDur, SUM(CASE WHEN result='Missed' THEN 1 ELSE 0 END) as missed FROM call_logs WHERE agent_id=? AND DATE(start_time)=? AND direction='Inbound'`,[agentId,date]);
    // Outbound calls
    const outCalls=await get(`SELECT COUNT(*) as total, AVG(duration) as avgDur FROM call_logs WHERE agent_id=? AND DATE(start_time)=? AND direction='Outbound'`,[agentId,date]);
    results.push({
      ...
      toggleCount,
      inboundCalls:inCalls.total||0,
      outboundCalls:outCalls.total||0,
      missedCalls:inCalls.missed||0,
      ahtInbound:Math.round(inCalls.avgDur||0),
      ahtOutbound:Math.round(outCalls.avgDur||0),
      totalCalls:(inCalls.total||0)+(outCalls.total||0)
    });
  }
  return results;
}
