...
// ── Thin wrappers matching the old sqlite3 promise API ────────
function run(sql, params = []) {
  db.run(sql, params);
  saveDB();
  // Mimic sqlite3's `this` result (lastID via last_insert_rowid)
  const row = db.exec('SELECT last_insert_rowid() as id')[0];
  const lastID = row ? row.values[0][0] : null;
...
function addAgent(name, extension, email) {
  return run(`INSERT OR IGNORE INTO monitored_agents (name,extension,email) VALUES (?,?,?)`, [name, extension, email || null]);
}
function removeAgent(extension) {
  return run(`DELETE FROM monitored_agents WHERE extension=?`, [extension]);
}
function getMonitoredAgents() {
  return all(`SELECT * FROM monitored_agents ORDER BY name ASC`);
}
function updateAgentRcId(extension, rcId) {
  return run(`UPDATE monitored_agents SET rc_id=? WHERE extension=?`, [rcId, extension]);
}
...
async function getPresenceEvents(date) {
  ...
  return all(
    `SELECT * FROM presence_events WHERE DATE(timestamp)=? AND agent_id IN (${rcIds.map(() => '?').join(',')}) ORDER BY timestamp ASC`,
    [date, ...rcIds]
  );
}
...
function insertCallLog(log) {
  return run(
    `INSERT OR IGNORE INTO call_logs (agent_id,agent_name,call_id,direction,result,duration,start_time) VALUES (?,?,?,?,?,?,?)`,
    [log.agentId, log.agentName, log.callId, log.direction, log.result, log.duration, log.startTime]
  );
}
...
async function getAgentSummary(date) {
  ...
  for (const agent of agents) {
    ...
    const events = await all(
      `SELECT status,timestamp FROM presence_events WHERE agent_id=? AND DATE(timestamp)=? ORDER BY timestamp ASC`,
      [agentId, date]
    );
    ...
    const calls = await get(
      `SELECT COUNT(*) as total, SUM(duration) as totalDuration, SUM(CASE WHEN result='Missed' THEN 1 ELSE 0 END) as missed FROM call_logs WHERE agent_id=? AND DATE(start_time)=?`,
      [agentId, date]
    );
    results.push({
      agentId,
...
function insertLoginLog(username, email, role, ip, location, systemInfo) {
  return run(
    `INSERT INTO login_logs (username,email,role,ip,location,system_info) VALUES (?,?,?,?,?,?)`,
    [username, email, role, ip || null, location || null, systemInfo || null]
  );
}
function getLoginLogs() {
  return all(`SELECT * FROM login_logs ORDER BY logged_in_at DESC LIMIT 500`);
}

// ── Role functions ────────────────────────────────────────────
function getAllRoles() {
  return all(`SELECT * FROM app_roles ORDER BY role ASC, email ASC`);
}
function setRole(email, role, addedBy) {
  return run(
    `INSERT INTO app_roles (email,role,added_by) VALUES (?,?,?) ON CONFLICT(email) DO UPDATE SET role=excluded.role,added_by=excluded.added_by`,
    [email, role, addedBy]
  );
}
function removeRole(email) {
  return run(`DELETE FROM app_roles WHERE email=?`, [email]);
}
async function getRoleForEmail(email) {
  const row = await get(`SELECT role FROM app_roles WHERE email=?`, [email]);
  return row ? row.role : null;
}

module.exports = {
  initDB, addAgent, removeAgent, getMonitoredAgents, updateAgentRcId,
  insertPresenceEvent, getPresenceEvents, insertCallLog, getAgentSummary,
  insertLoginLog, getLoginLogs, getAllRoles, setRole, removeRole, getRoleForEmail
};